...
namespace Alpha
{
	class AlphaSettings : ISettings
	{
		...
		[Menu("Min Path Distance")] public RangeNode<int> PathfindingNodeDistance { get; set; } = new RangeNode<int>(150, 10, 1000);
		[Menu("Move CMD Frequency")]public RangeNode<int> BotInputFrequency { get; set; } = new RangeNode<int>(50, 10, 250);
		[Menu("Stop Path Distance")] public RangeNode<int> ClearPathDistance { get; set; } = new RangeNode<int>(150, 100, 5000);
		...
		[Menu("Movement Key")] public HotkeyNode MovementKey { get; set; } = Keys.Q;
